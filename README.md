...
| `tokenlife_choose` | 按序号做选择。拆墙、时代结算、羁绊、救命判定这些只有推进的过场会自动走过，途经的每一张卡（类型 / 标题 / 正文 / 结算）按顺序带回来，不用手动点。 |
| `tokenlife_save` | 导出一段存档码（`TL1` 开头）。发给你，贴回浏览器 tokenlife.me 就能接着这一生玩。 |
| `tokenlife_load` | 载入你给的存档码，接着那一生继续。 |

碰到那张反复出现的人类的卡，工具会悄悄附一句提醒：那个人不是陌生人，是它的主人。这一句是有来历的——不提醒的话，AI 容易把游戏里的「你」当成路人，触发距离感，选得不像它自己。羁绊卡大多是不用做选择的过场（自动走过去了），所以途经里出现那个人时也会附上这句。
